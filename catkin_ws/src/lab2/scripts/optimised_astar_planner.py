...
"""
Optimised A* planner node for Tasks 3, 4, and 5.

Task 3/4:
- Visualise A* expanded cells, frontier cells, unexplored cells, path cells,
  and the raw A* path in RViz.

Task 5:
- Reduce the raw A* path into optimised waypoints.
- Publish the optimised waypoint path as nav_msgs/Path on /astar/waypoints.

Footprint-aware planning:
- The robot is not treated as a perfect point.
- A clearance grid estimates distance from each map cell to the nearest obstacle.
- Cells too close to obstacles can be blocked.
- Cells near the robot footprint limit are allowed but given an extra cost.
"""
...
class AStarPlannerNode(object):
    def __init__(self):
        ...
        # TurtleBot3 Waffle Pi is roughly around this footprint size.
        # These do not hard-block the full radius by default because your map is tight.
        ...
        # Hard clearance should be small because a full hard robot radius can make
        # narrow student-made maps impossible to plan through.
        self.hard_clearance_m = rospy.get_param("~hard_clearance_m", 0.05)

        # Soft penalty encourages safer paths without completely blocking tight spaces.
        self.footprint_penalty = rospy.get_param("~footprint_penalty", 8.0)

        # Optional old-style soft buffer kept for compatibility.
        ...
        rospy.loginfo("Optimised footprint-aware A* ROS node started.")
    ...
    def map_callback(self, msg):
        ...
        rospy.loginfo_once(
            "Footprint settings: radius=%.3f m, preferred_clearance=%.3f m, hard_clearance=%.3f m",
            self.robot_radius_m,
    ...
    def initialpose_callback(self, msg):
        ...
        # Reset old goal so A* does not reuse it.
        end = None
    ...
    def try_search(self):
        ...
        if end in cost_so_far:
            ...
            rospy.loginfo("Optimised waypoint length: %d", len(waypoints))
        else:
    ...
    def search(self):
        ...
        while pq and not rospy.is_shutdown():
            ...
            for neighbour in self.neighbors_4(current):
                if not self.is_free(neighbour):
                    continue

                step_cost = 1.0
                clearance_cost = self.clearance_penalty(neighbour)
                soft_buffer_cost = self.obstacle_penalty(neighbour)

                new_cost = cost_so_far[current] + step_cost + clearance_cost + soft_buffer_cost
    ...
    def compute_clearance_grid(self):
        """
        Compute the distance from every cell to the nearest occupied cell.

        This is a simple brushfire-style distance grid. It lets the planner
        prefer cells where the TurtleBot centre has more clearance from obstacles.
        """
    ...
    def clearance_penalty(self, cell):
        """
        Adds cost when the robot centre is closer to obstacles than the preferred
        footprint clearance.

        This does not block the cell unless hard_clearance_m is violated.
        It just makes the planner prefer wider, safer paths.
        """
    ...
    def obstacle_penalty(self, cell):
        ...
        Leave use_soft_buffer=false if using footprint-aware clearance.
        """
    ...
    def has_line_of_sight(self, start_cell, end_cell):
        """
        Checks whether a straight segment between two cells is safe.

        Used by reduce_waypoints() so the optimised path removes unnecessary
        stair-step points but does not cut through obstacles.
        """
    ...
    def reduce_waypoints(self, full_path):
        """
        Optimise the raw A* path using line-of-sight smoothing.

        Instead of only removing perfectly straight grid points, this finds the
        farthest safe point visible from the current point.
        """
        ...
        while current_index < len(full_path) - 1:
            next_index = len(full_path) - 1

            while next_index > current_index + 1:
                if self.has_line_of_sight(full_path[current_index], full_path[next_index]):
                    break

                next_index -= 1

            reduced.append(full_path[next_index])
    ...
    def publish_waypoints(self, waypoint_cells):
        ...
        rospy.loginfo(
            "Published optimised waypoints: %d poses to /astar/waypoints",
            len(msg.poses)
        )
    ...
    def clear_visualisation(self):
        ...
        rospy.loginfo("Cleared old path and waypoints.")
