...
def plot_joint_positions(csv_file):
    ...
    plt.title(
        "Task 9: OpenManipulator-X Joint Positions Over Time"
    )
    ...
    print(
        "[SUCCESS] Saved: "
        + os.path.abspath("task9_joint_positions.png")
    )
...
def plot_end_effector_3d(csv_file):
    ...
    axes.set_title(
        "Task 9: Reconstructed End-Effector Path"
    )
    ...
    plt.savefig(
        "task9_end_effector_3d_path.png",
        dpi=300,
    )
...
def plot_tracking_error(actual_csv, desired_csv):
    ...
    plt.title(
        "Task 9: Cartesian Trajectory Tracking Error"
    )
    ...
    print(
        "[SUCCESS] Saved: "
        + os.path.abspath("task9_tracking_error.png")
    )

    print("")
    print("--- Task 9 Tracking Error Metrics ---")
...
if __name__ == "__main__":
    plot_joint_positions(
        "../data/task9_actual_joint_positions.csv"
    )

    plot_end_effector_3d(
        "../data/task9_actual_end_effector.csv"
    )

    plot_tracking_error(
        "../data/task9_actual_end_effector.csv",
        "../data/task9_desired_taskspace.csv",
    )